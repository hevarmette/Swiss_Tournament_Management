...
def delta_str(delta: float) -> str:
    """Format an Elo delta with a leading ± sign and colour."""
    if delta >= 0:
        return f"{C.GREEN}+{delta:+.1f}{C.RESET}"
    return f"{C.RED}{delta:+.1f}{C.RESET}"
...
def draw_match(grid, row_top, row_bot, col_in, col_out,
               top_name, bot_name, winner_name):
...
class Tournament:
    ...
    def record_match(self, p1_id, p2_id, p1_won, round_number):
        ...
        self.round_deltas[round_number].append({
            "p1": p1_id, "p2": p2_id,
            "p1_won": p1_won,
            "p1_before": p1_elo_before, "p2_before": p2_elo_before,
    ...
    def generate_pairings(self, round_number, phase1_rounds, minimum_match_points):
        ...
        for record in sorted_records:
            ...
            while len(group) >= 2:
                p1 = group.pop(0)
                partner_idx = next(
                    (j for j, cand in enumerate(group) if self.can_pair(p1, cand)),
                    None
                )
    ...
    def print_round_results(self, round_number: int):
        ...
        for d in deltas:
            ...
            print(
                f"  {C.GREEN}{win:<14}{C.RESET}"
                f"{C.DIM}def.{C.RESET}"
                f"  {C.RED}{lose:<14}{C.RESET}"
                f"  {delta_str(d1):>12}  {C.DIM}{e1:>6.0f}{C.RESET}"
                f"  {delta_str(d2):>12}  {C.DIM}{e2:>6.0f}{C.RESET}"
            )
    ...
    def print_standings(self, top_cut, long_line=72, short_line=70):
        ...
        for i, row in standings.iterrows():
            ...
            print(
                f"  {medal}  {col}{row['name']:<16}{C.RESET}"
...
class SingleEliminationBracket:
    def __init__(self, seeded_player_ids: List[str],
                 tournament: Tournament, registry: PlayerRegistry):
    ...
    def get_current_matches(self) -> List[BracketMatch]:
        return [m for m in self.rounds.get(self.current_round, [])
    ...
    def record_result(self, round_number, match_number, winner_id):
        ...
        loser_id = target.player2_id if winner_id == target.player1_id else target.player1_id
    ...
    def print_bracket(self):
        ...
        for i in range(self.num_rounds + 1):
            if i == self.num_rounds:         header = "Champion"
            elif i == self.num_rounds - 1:   header = "Finals"
            elif i == self.num_rounds - 2:   header = "Semifinals"
            elif i == self.num_rounds - 3:   header = "Quarters"
            else:                            header = f"Round {i+1}"
        ...
        for r in range(self.num_rounds):
            ...
            for m_visual_idx, match_num in enumerate(visual_match_nums):
                match = next(
                    (m for m in self.rounds.get(swiss_round_num, [])
                ...
                if match:
                    if swiss_round_num == 1:
                        p1_name = (self.tournament.players[match.player1_id].name
                                   if match.player1_id else "")
                        if match.is_bye:
                            p2_name     = "(BYE)"
                            winner_name = p1_name
                        else:
                            p2_name     = (self.tournament.players[match.player2_id].name
                                           if match.player2_id else "")
                            winner_name = (self.tournament.players[match.winner_id].name
                                           if match.winner_id else "")
                    else:
                        prev_rn       = swiss_round_num - 1
                        prev_visuals  = get_visual_match_numbers(prev_rn, self.num_rounds)
                        top_match = next(
                            (m for m in self.rounds.get(prev_rn, [])
                        ...
                        p1_name     = (self.tournament.players[top_id].name
                                       if top_id else "")
                        p2_name     = (self.tournament.players[bot_id].name
                                       if bot_id else "")
                        winner_name = (self.tournament.players[match.winner_id].name
                                       if match.winner_id else "")

                draw_match(grid, row_top, row_bot, col_in, col_out,
                           p1_name, p2_name, winner_name)
...
def print_elo_insights(registry: PlayerRegistry, tournament: Tournament):
    ...
    for entry, delta in movers[:5]:
        ...
        print(
            f"  {elo_color(entry.elo)}{entry.name:<18}{C.RESET}"
            f"  {entry.elo_history[0]:>6.0f} → {entry.elo:>6.0f}"
            f"  {delta_str(delta):>14}  {bar}"
        )
    ...
    for rn, deltas in tournament.round_deltas.items():
        for d in deltas:
            ...
            loser_was_fav = (
                (d["p1_before"] > d["p2_before"] and not d["p1_won"]) or
    ...
    if biggest_upset:
        ...
        print(
            f"  {C.DIM}Elo gap: {C.RESET}{C.YELLOW}{biggest_gap:.0f} points{C.RESET}"
        )

    # Distribution
    bins = {"≥1800": 0, "1650–1799": 0, "1500–1649": 0,
            "1350–1499": 0, "1200–1349": 0, "<1200": 0}
    for e in entries:
...
if __name__ == "__main__":
    ...
    print(f"{C.BOLD}{C.GOLD}{'█'*width}{C.RESET}")
    print(f"{C.DIM}  {len(players)} players  │  {total_rounds} rounds  │"
    ...
    if champ_id:
        ...
        print(f"{C.BOLD}{C.GOLD}{'🏆':^5} CHAMPION: "
